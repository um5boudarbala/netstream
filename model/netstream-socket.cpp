 ...
 ** under the terms of the BSD License. Refer to the ns3-licence.txt **
...
#ifdef NS3
    extern "C" void init_tcpip( ns3::SimulationController& sc )
    {
...
namespace ns3
{
		...
		const 
	{
		...
		if (r < 0)
			BailOnNetStreamSocketError("ns3::NetStreamSocket::datawaiting @ select");
		...
		
	{
		...
		if( server_socket_ < 0 )
		{
			...
			if( server_socket_ < 0 )
				BailOnNetStreamSocketError("ns3::NetStreamSocket::accept() @ socket");
			...
			if ( ::bind(server_socket_, (struct sockaddr*)&self, sizeof(self)) != 0 )
				BailOnNetStreamSocketError("ns3::NetStreamSocket::accept() Unable to create listening socket");


			// Make it a "listening socket"
			if ( listen(server_socket_, 10) == -1 )
				BailOnNetStreamSocketError("ns3::NetStreamSocket::accept() Unable to listen on server socket");
		...
		
	{
		...
		if( server_socket_ > 0 )
		{
		    ...
		    if (ioctlsocket(server_socket_, FIONBIO, &NonBlock) == SOCKET_ERROR)
				BailOnNetStreamSocketError("ns3::NetStreamSocket::set_blocking() Unable to initialize non blocking I/O");
#else
		...
		
	{
		...
		if( !atoaddr( host_.c_str(), addr) )
			BailOnNetStreamSocketError("ns3::NetStreamSocket::connect() @ Invalid network address");
		...
		if( socket_ < 0 )
			BailOnNetStreamSocketError("ns3::NetStreamSocket::connect() @ socket");

		if( ::connect( socket_, (sockaddr const*)&address, sizeof(address) ) < 0 )
			BailOnNetStreamSocketError("ns3::NetStreamSocket::connect() @ connect");
		...
		
	{
		...
		if (verbose_) 
		{
			cerr << "Send " << numbytes << " bytes via ns3::NetStreamSocket: [";
			for(size_t i = 0; i < numbytes; ++i)
			{
				buf[i] = b[i];
				cerr << " " << (int)b[i] << " ";
		...
		
	{
		...
		if( a <= 0 )
		{
			// BailOnNetStreamSocketError definitely throws an exception so clear up heap
			delete[] buf;
			BailOnNetStreamSocketError( "ns3::NetStreamSocket::receive() @ recv" );
		}
		...
		if (verbose_) 
		{
			cerr << "Rcvd "  << a <<  " bytes via ns3::NetStreamSocket: [";
			for(int i = 0; i < a; ++i)
			{
				cerr << " " << (int)b[i] << " ";
			}
		...
		
	{
		...
		if (verbose_)
		{
			cerr << "Rcvd Storage with "  << 4 + NN <<  " bytes via ns3::NetStreamSocket: [";
			for (int i=0; i < 4; ++i)
			{
		...
		const
	{
		...
		case WSAECONNABORTED:	return "Software caused ns3::NetStreamSocket abort";
